        ...
        for (int j = 0; j < m; ++j) {
            ...
            if(c[i][j][2] != c[i][j+1][0] && j+1<m){ 
                ...
                if(i>0){//norte de c'
                    ...
                    clausulas.push_back(f+" "+to_string(c[i-1][j][3])+" 0");

                    printf("c''(%d,%d)\n", i-1,j+1);
                    printf("-%d -%d %d %d\n",q0,q1,c[i][j+1][0],q0);
                    printf("-%d -%d %d %d\n",q0,q1,c[i][j+1][0],c[i-1][j][3]);  
                }
                if(i+1<n){ //sur de c'
                    q0 = c[i][j][0];
                    q1 = c[i][j+1][1];
            ...
            if(j>0 && c[i][j][2] != c[i][j-1][0]){ 
                printf("\n---c'(%d,%d)\n",i,j-1);
                if(i>0){//norte de c'
                    q0 = c[i][j][2];
                    q1 = c[i][j][1];
                    f = "-"+to_string(q0)+" -"+to_string(q1) +" "+to_string(c[i][j][0]);
                    clausulas.push_back(f+" "+to_string(q0)+" 0");
                    clausulas.push_back(f+" "+to_string(c[i-1][j][2])+" 0");

                    printf("c''(%d,%d)\n", i-1,j-1);
                    printf("-%d -%d %d %d\n",q0,q1,c[i][j][0],q0);
                    printf("-%d -%d %d %d\n",q0,q1,c[i][j][0],c[i-1][j-1][3]);  
                }
                if(i+1<n){ //sur de c'
                    q0 = c[i][j][0];
                    q1 = c[i][j][1];
